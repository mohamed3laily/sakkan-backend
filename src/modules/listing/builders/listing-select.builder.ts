...
export class ListingSelectBuilder {
  static getSelectFields(userId?: number) {
    ...
    `.as('areas'),
      ...
      isFavorited: userId
        ? sql<boolean>`EXISTS (SELECT 1 FROM favorites WHERE favorites.user_id = ${userId} AND favorites.favoritable_type = 'LISTING' AND favorites.favoritable_id = ${listings.id})`.as('isFavorited')
